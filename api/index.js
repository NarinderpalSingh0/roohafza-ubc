...
// server/newsletter.ts
var newsletterSubscriptionInput = z2.object({
  email: z2.string().trim().toLowerCase().email("Enter a valid email address.").max(320)
});
async function subscribeToNewsletter(input) {
  const db = await getDb();
  if (!db) {
    throw new Error("Newsletter subscriptions are temporarily unavailable.");
  }
  await db.insert(newsletterSubscribers).values({ email: input.email, source: "homepage" }).onDuplicateKeyUpdate({ set: { source: "homepage" } });
...
// server/routers/commerce.ts
var cartLineInputSchema = z3.object({
  variantId: z3.string().min(1),
  quantity: z3.number().int().min(1).max(99)
});
var cartLineUpdateSchema = z3.object({
  lineId: z3.string().min(1),
  /** 0 means "remove this line" — the route forwards to removeLines. */
  quantity: z3.number().int().min(0).max(99)
});
var commerceRouter = router({
  products: router({
    list: publicProcedure.input(
      z3.object({
        first: z3.number().int().min(1).max(100).optional(),
        collectionHandle: z3.string().min(1).optional()
      }).optional()
    ).query(async ({ input }) => {
      return listProducts(input ?? {});
    }),
    byHandle: publicProcedure.input(z3.object({ handle: z3.string().min(1) })).query(async ({ input }) => {
      return getProductByHandle(input.handle);
    })
  }),
  collections: router({
    list: publicProcedure.input(z3.object({ first: z3.number().int().min(1).max(50).optional() }).optional()).query(async ({ input }) => {
  ...
  cart: router({
    create: publicProcedure.input(z3.object({ lines: z3.array(cartLineInputSchema).min(1).max(50) })).mutation(async ({ input }) => {
      return createCart(input.lines);
    }),
    get: publicProcedure.input(z3.object({ cartId: z3.string().min(1) })).query(async ({ input }) => {
      return getCart(input.cartId);
    }),
    addLines: publicProcedure.input(
      z3.object({
        cartId: z3.string().min(1),
        lines: z3.array(cartLineInputSchema).min(1).max(50)
      })
    ).mutation(async ({ input }) => {
    ...
    updateLines: publicProcedure.input(
      z3.object({
        cartId: z3.string().min(1),
        lines: z3.array(cartLineUpdateSchema).min(1).max(50)
      })
    ).mutation(async ({ input }) => {
    ...
    removeLines: publicProcedure.input(
      z3.object({
        cartId: z3.string().min(1),
        lineIds: z3.array(z3.string().min(1)).min(1).max(50)
      })
    ).mutation(async ({ input }) => {
